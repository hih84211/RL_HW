...
#
weight_hid=(0.25*torch.randn(size=(4,4), dtype=torch.float32)).clone().detach().requires_grad_(True)
bias_hid=torch.tensor([0.0]*4, dtype=torch.float32, requires_grad=True)
weight_out=(0.25*torch.randn(size=(4,3), dtype=torch.float32)).clone().detach().requires_grad_(True)
