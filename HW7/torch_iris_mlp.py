...
data_train, data_test, target_train, target_test = train_test_split(ss_iris_data, iris_target, test_size=0.25,
                                                                    random_state=11, stratify=iris_target)
...
#
weight_hid = (0.25 * torch.randn(size=(4, 3), dtype=torch.float32)).clone().detach().requires_grad_(True)
bias_hid = torch.tensor([0.0] * 3, dtype=torch.float32, requires_grad=True)
weight_out = (0.25 * torch.randn(size=(3, 3), dtype=torch.float32)).clone().detach().requires_grad_(True)
